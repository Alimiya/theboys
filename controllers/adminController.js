    ...
    try {
        if (new_attraction_img != undefined) {
            ...
            await storage.upload(new_attraction_img.path, { 
                gzip: true,
                metadata: metadata,
                destination: `attractions/${attraction_id + random}`
            });
        ...
        if (new_attraction_img != undefined) {
            updateData.attraction_img = `https://firebasestorage.googleapis.com/v0/b/smart-guide-d28d6.appspot.com/o/attractions%2F${attraction_id + random}?alt=media`
        }
